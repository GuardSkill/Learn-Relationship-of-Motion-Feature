...
from models.resnext import Attention,fusion_model
...
def test():
    ...
    val_dataloader = DataLoader(data, batch_size=1, shuffle=False, num_workers=opt.n_workers, pin_memory=True,
                                drop_last=False)
    ...
    model_fusion=fusion_model(opt.n_classes)
    ...
    if opt.log:
        f = open(os.path.join(root_dir,
                        "test_Fusion_{}{}_{}_{}_{}.txt".format(opt.model, opt.model_depth, opt.dataset,
                                                                           opt.split, opt.sample_duration)), 'w+')
        f.write(str(opt))
        f.write('\n')
        f.flush()
    ...
    with torch.no_grad():
        for i, (clip, label) in enumerate(val_dataloader):
            ...
            RGB_clip = clip[0:3, :, :, :]
            Flow_clip = clip[3:, :, :, :]
            inputs1 = torch.Tensor(int(RGB_clip.shape[1] / opt.sample_duration), 3, opt.sample_duration,
                                   opt.sample_size, opt.sample_size)
            inputs2 = torch.Tensor(int(Flow_clip.shape[1] / opt.sample_duration), 2, opt.sample_duration,
                                   opt.sample_size, opt.sample_size)
            ...
            outputs_var1= torch.mean(outputs_var1, dim=0).unsqueeze(0)
            outputs_var2=  torch.mean(outputs_var2, dim=0).unsqueeze(0)
            # outputs_var = torch.mean(torch.cat((outputs_var1, outputs_var2), dim=0), dim=0).unsqueeze(0)
            _,outputs_var=model_fusion(outputs_var1, outputs_var2)
            ...
            line = "Video[" + str(i) + "] : \t top5 " + str(pred5) + "\t top1 = " + str(pred5[0]) + "\t true = " + str(
