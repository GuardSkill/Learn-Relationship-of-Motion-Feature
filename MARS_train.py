...
from models.loss import ModelL1Loss, ModelGramLoss
...
if __name__=="__main__":
    ...
    loss_flag=False
    for i in opt.output_layers:
        if i == 'dict':
            criterion_Flow_Gram=ModelGramLoss().cuda()
            criterion_Flow_L1 = ModelL1Loss().cuda()
            loss_flag=True
    ...
    for epoch in range(opt.begin_epoch, opt.n_epochs + 1):
        ...
        for i, (inputs, targets) in enumerate(train_dataloader):
            ...
            if loss_flag:
                loss_MARS = criterion_MARS(outputs_MARS['x7'], targets)
            else:
                loss_MARS = criterion_MARS(outputs_MARS[0], targets)
            if loss_flag:
                for key in outputs_Flow:
                    outputs_Flow[key]=outputs_Flow[key].detach()
                loss_MSE = opt.MARS_alpha * (criterion_Flow_Gram(outputs_MARS, outputs_Flow)+criterion_Flow_L1(outputs_MARS,outputs_Flow))
            else:
                loss_MSE = opt.MARS_alpha*criterion_Flow(outputs_MARS[1], outputs_Flow[1].detach())
            loss     = loss_MARS + loss_MSE
            if loss_flag:
                acc = calculate_accuracy(outputs_MARS['x7'], targets)
            else:
        ...
        with torch.no_grad():
            for i, (inputs, targets) in enumerate(val_dataloader):
                ...
                outputs_MARS  = model_MARS(inputs_MARS)
                if loss_flag:
                    loss = criterion_MARS(outputs_MARS['x7'], targets)
                else:
